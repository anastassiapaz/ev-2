# ev-2[index.html](https://github.com/user-attachments/files/27179819/index.html)
<!doctype html>
<html lang="es">
    <head>
        <meta charset="utf-8" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <title>PROYECTOS DE TITULO DE DISENO</title>
        <style>
            *,
            *::before,
            *::after {
    ...
    <body>
        <div id="contenedor">
            ...
            <input type="text" id="elInput" placeholder="Filtrar proyectos de titulo..." />
        ...
        <script>
            ...
            const URL = "https://api.myjson.online/v1/records/176f7cb9-0aac-4765-a0b2-4d9832074f0d";
            ...
            fetch(URL)
                ...
                .then((datos) => {
                    ...
                    console.log(trabajo);
                    trabajo.forEach((x, i) => {
                        const proyecto = x.repository
                            ? `<a href="${x.repository}" target="_blank" rel="noopener noreferrer">${x.title}</a>`
                            : x.title;
                        t.innerHTML += `<tr><td>${i + 1}</td><td>${x.name}</td><td>${proyecto}</td><td>${x.grade}</td><td>${x.category}</td><td>${x.tutor}</td></tr>`;
                    });
                })
                .catch((error) => {
